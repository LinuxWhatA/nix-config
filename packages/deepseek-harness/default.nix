...
stdenv.mkDerivation (finalAttrs: {
  ...
  pnpmDeps = fetchPnpmDeps {
    ...
    pnpm = pnpm_11;
    fetcherVersion = 4;
    # 到 registry.npmjs.org 的链路会周期超时（pnpm 错误码 23），加大单请求
    # 超时/重试，让大量小包的下载能在抖动的链路上完成。
    # fetch 脚本硬编码 --force（全平台拉取以便跨架构复现）；meta.platforms
    # 只声明 x86_64-linux，用 --no-force 反转：pnpm 默认只拉当前宿主平台
    # （linux-x64-glibc），体积约省一半。
    pnpmInstallFlags = [
    ...
    timeout = 3600; # 1小时超时
    retry = 5; # 重试5次
    hash = "sha256-yAPK8vHXW/4XGKWTSI6aSgd1WUMxJHRu0UKhGOesIOk=";
  };
